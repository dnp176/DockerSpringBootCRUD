...
document.getElementById("cpuStressBtn").addEventListener("click", () => {
    fetch(`${apiBaseUrl}/stress-cpu`, { method: "GET" })
        .then(response => {
            if (response.ok) {
                alert("CPU stress initiated. Check your system utilization.");
            } else {
                alert("Error initiating CPU stress.");
            }
        })
        .catch(error => console.error("Error:", error));
});
